...
async def teardown_request(exc=None):
    """Always cleanup database session, even on exceptions"""
    if hasattr(request, 'db_session'):
        try:
            if exc:
                # Rollback on error
                await request.db_session.rollback()
            await request.db_session.close()
        except Exception:
...
async def search():
    ...
    return await render_template("search_foods.html",
                                 query=query,
                                 api_key=api_key)
...
async def login():
    ...
    if request.method == "POST":
        ...
        try:
            ...
            result = await request.db_session.execute(stmt)
...
async def register():
    ...
    if request.method == "POST":
        ...
        error_message = validate_registration_form(username, email, password,
                                                   confirm_pass)
        if error_message:
...
async def food_log():
    ...
    if request.method == "POST":
        ...
        if (calorie is not None and protein is not None and carbs is not None
                and fat is not None):
            try:
            ...
            if (not is_float(calorie) or not is_float(protein)
                    or not is_float(carbs) or not is_float(fat)):
                return await apology("Error invalid values!", 400)

            if (int(calorie) < 0 or float(protein) < 0 or float(carbs) < 0
                    or float(fat) < 0):
        ...
        if food:
            ...
            await request.db_session.execute(stmt)
            await request.db_session.commit()
    ...
    # if the user reached GET (as by clicking food_log)
        ...
        prev_sunday = selected_date - timedelta(days=selected_date.weekday() +
                                                1)
        week_dates = []
        ...
        if food_log_query:
            ...
            bars_calories = ax.bar(xpos - 0.1,
                                   total_calories,
                                   width=0.4,
                                   label="Calories")
            bars_protein = ax.bar(xpos - 0.1,
                                  total_protein,
                                  width=0.4,
                                  label="protein")
            bars_carbs = ax.bar(xpos + 0.2,
                                total_carbs,
                                width=0.2,
                                label="Carbs")
            ...
            # Convert chart to base64 image (much faster than animation)
            buffer = BytesIO()
            plt.tight_layout()
            ...
            # Create simple HTML img tag
            graph_html = f'<img src="data:image/png;base64,{image_base64}" alt="Weekly Nutrition Chart" style="max-width: 100%; height: auto;">'

        # Get today's totals for circular charts
        today = datetime.now()
        today_stmt = select(
            func.sum(FoodCount.calories).label("total_calories"),
            func.sum(FoodCount.protein).label("total_protein"),
            ...
            FoodCount.user_id == session["user_id"],
            FoodCount.month == today.month,
            FoodCount.day == today.day,
            FoodCount.year == today.year,
        )
        today_result = await request.db_session.execute(today_stmt)
        today_totals = today_result.fetchone()

        return await render_template("food-log.html",
                                     food_log=food_log,
                                     graph_html=graph_html,
                                     selected_date=selected_date_str,
                                     today_totals={
                                         'calories':
                                         float(today_totals.total_calories
                                               or 0),
                                         'protein':
                                         float(today_totals.total_protein
                                               or 0),
                                         'carbs':
                                         float(today_totals.total_carbs or 0),
                                         'fat':
                                         float(today_totals.total_fat or 0)
                                     })
...
async def generate():
    ...
    if model is None:
        return jsonify({
            "error":
            "AI service is currently unavailable. Please set up your Gemini API key."
        }), 503
